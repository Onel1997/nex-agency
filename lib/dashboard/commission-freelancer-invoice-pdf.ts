...
import { COMMISSION_PAYOUT_ROLE_LABELS } from "./commission-freelancer-invoice-constants";
import { formatCents, formatDate } from "./format";
import { INVOICE_COMPANY } from "./invoice-company";
import { registerInvoicePdfFonts } from "./invoice-pdf-fonts";
import type { CommissionFreelancerInvoiceWithDetails } from "./types";
...
function renderFreelancerBlock(
  doc: InstanceType<typeof PDFDocument>,
  fonts: { regular: string; bold: string },
  invoice: CommissionFreelancerInvoiceWithDetails,
): number {
...
function renderServiceBlock(
  ...
  doc.text(invoice.service_description, PAGE.margin, y, {
    width: PAGE.width - PAGE.margin * 2,
  });
  y +=
    doc.heightOfString(invoice.service_description, {
      width: PAGE.width - PAGE.margin * 2,
    }) + 12;
  ...
  y = renderLabelValueLine(
    ...
    280,
    "Rolle",
    COMMISSION_PAYOUT_ROLE_LABELS[invoice.role],
  );
...
export async function generateCommissionFreelancerInvoicePdfBuffer(
  invoice: CommissionFreelancerInvoiceWithDetails,
): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      const serviceEndY = renderServiceBlock(doc, fonts, invoice, recipientBottomY);
      renderPaymentBlock(doc, fonts, invoice, serviceEndY);
      doc.end();
    } catch (error) {
      reject(error);
    }
