...
import { payCommission } from "@/app/dashboard/finance/actions";
...
export function CommissionPayoutModal({
  client,
  open,
  onClose,
}: CommissionPayoutModalProps) {
  ...
  useEffect(() => {
    ...
    setAmount(centsToEuroInput(client.commission_outstanding_cents));
    setError(null);
  }, [client, open]);
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      try {
        await payCommission(client.id, formData);
        router.refresh();
        onClose();
      } catch (err) {
        setError(
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title="Provision auszahlen"
      layer="stacked"
      size="lg"
    >
      <dl className="mb-5 grid gap-3 sm:grid-cols-2">
        <InfoItem label="Mitarbeiter" value={client.responsible_member_name ?? "—"} />
