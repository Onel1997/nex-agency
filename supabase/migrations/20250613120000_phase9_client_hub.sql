...
-- =============================================================================
-- Backfill client_activities from existing leads/clients
-- =============================================================================
...
WHERE NOT EXISTS (
  SELECT 1 FROM public.client_activities ca
  WHERE ca.client_id = c.id AND ca.activity_type = 'lead_created'
);
...
WHERE (l.status = 'won' OR l.converted_to_client = true)
  AND NOT EXISTS (
    SELECT 1 FROM public.client_activities ca
    WHERE ca.client_id = c.id AND ca.activity_type = 'lead_won'
  );
...
WHERE NOT EXISTS (
  SELECT 1 FROM public.client_activities ca
  WHERE ca.client_id = c.id AND ca.activity_type = 'client_created'
);
...
WHERE al.entity_type = 'client'
  ...
  AND NOT EXISTS (
    SELECT 1 FROM public.client_activities ca
    WHERE ca.client_id = al.entity_id
